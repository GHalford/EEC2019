...
for (k in kk){
  ...
  RDegree<-rpt(Degree~(1|ID), grname="ID", data=Result_Dataset, nboot=1000)
  rD<-data.frame(NULL)
  rD<-as.numeric(RDegree$R)
  
  RClose<-rpt(Closeness~(1|ID), grname="ID", data=Result_Dataset, nboot=1000)
  rC<-data.frame(NULL)
  rC<-as.numeric(RClose$R)
  
  RBet<-rpt(Betweenness~(1|ID), grname="ID", data=Result_Dataset, nboot=1000)
  rB<-data.frame(NULL)
...
#save data 
write.csv(RData, "NetworkSimulationData.csv")
head(RData)
#plot null model distribution with observed values and confidence intevals 
plot(density(RData$RBet), xlim=c(0,0.5), main="", xlab="Repeatability Betweeness")
lines(x=c(0.287,0.287), y=c(0,32), col="red")
#lines(x=c(0.000,0.000), y=c(0,27), col="red", lty=2)
#lines(x=c(0.536,0.536), y=c(0,27), col="red", lty=2)

plot(density(RData$RClose), xlim=c(0,0.5), main="", xlab="Repeatability Closeness")
lines(x=c(0.232,0.232), y=c(0,145), col="red")
#lines(x=c(0.00,0.00), y=c(0,135), col="red", lty=2)
#lines(x=c(0.459,0.459), y=c(0,135), col="red", lty=2)

plot(density(RData$RDeg), xlim=c(0,0.5), main="", xlab="Repeatability Degree")
lines(x=c(0.422,0.422), y=c(0,250), col="red")
